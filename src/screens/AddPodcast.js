...
const AddPodcast = ({route}) => {
  ...
  const baseImagePath = "podcasts/images/";
  const baseAudioPath = "podcasts/audios/";
  ...
  const submit = async () => {
    ...
    if (formComplete() && validateDuration() && validateCost()) 
    {
      ...
      const docRef = await dbh.collection("podcasts").add({
        title: title,
      ...
      uploadImageStatus.on(firebase.storage.TaskEvent.STATE_CHANGED, {
        'complete': async function() {
          ...
          const audioURI = (audio.uri.substr(0,7) != "file://") ? "file://" + audio.uri : audio.uri;
          // Code from: https://medium.com/@ericmorgan1/upload-images-to-firebase-in-expo-c4a7d4c46d06
          ...
          uploadAudioStatus.on(firebase.storage.TaskEvent.STATE_CHANGED, {
            ...
            'complete': function() {
              ...
              navigation.goBack();
              navigation.replace("PodcastList");
            },
            'error': function() {
              Alert.alert(
    ...
    <View style={styles.app}>
      {
        isTopicsLoaded && !isUploadInProgress ?
          <View style={styles.fullWidthWindow}>
            <Text style={styles.contentTitle}>Add Podcast</Text>
